...
import com.sd.tgfinger.tgApi.bigFeature.callBack.SoundPlay;
import com.sd.tgfinger.tgApi.bigFeature.common.Constant;
...
public class TGB2API {
    ...
    private static TGB2API INSTANCE = null;

    public synchronized static TGB2API getTGAPI() {
        if (INSTANCE == null) {
            INSTANCE = new TGB2API();
        }
        return INSTANCE;
    }
    ...
    //获取算法代理对象
    private TG_Big_FV getTGFV() {
        return TG_Big_FV.TGFV_INSTANCE;
    }
    ...
    private int workType = TGB2Constant.WORK_BEHIND;//默认是后比
    ...
    public void startDevService(Context context, OnStartDevStatusServiceListener statusServiceListener) {
        if (context != null) {
            ...
            intent.setAction(TGB2Constant.DEV_SERVICE_ACTION);
    ...
    private ServiceConnection serviceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName componentName, IBinder iBinder) {
            ...
            tg661JMessage.what = TGB2Constant.SEND_MESSAGE_CODE;
    ...
    private Messenger tg661JMessenger = new Messenger(new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == TGB2Constant.RECEIVE_MESSAGE_CODE) {
    ...
    private void sendDevStatusToView(Message msg) {
        ...
        if (data != null) {
            int devServiceArg = data.getInt(TGB2Constant.STATUS);
    ...
    public void openDev(@NonNull Integer workType, @NonNull Integer templModelType, boolean sound
            , @NonNull final DevOpenCallBack callBack
            , @NonNull DevStatusCallBack devStatusCallBack) {
        ...
        this.devStatusCallBack = devStatusCallBack;
        if (templModelType == TGB2Constant.TEMPL_MODEL_6) {
            isType6 = true;
        } else if (templModelType == TGB2Constant.TEMPL_MODEL_3) {
            isType6 = false;
        }
        if (executor != null) {
            executor.execute(new Runnable() {
...
//                    writeCMD();
    ...
    public void featureCompare1_N(@NonNull final byte[] verifyData, @NonNull final int verifySize,
        ...
        if (executor != null) {
            executor.execute(new Runnable() {
                @Override
                public void run() {
                    aloneVerifyN(verifyData, verifySize, TGB2API.this.sound, new VerifyMsg() {
                        @Override
    ...
    private void aloneVerifyN(@NonNull byte[] fingerTemplData, int fingerSize, Boolean isSound, VerifyMsg verifyMsg) {
        if (!TGB2API.this.continueVerify && fingerSize > 0) {
    ...
    private void tgGetTemplTime(byte[] templData, MsgCallBack msgCallBack) {
        byte[] snData = new byte[TGB2Constant.TIME_SIZE];
    ...
    private void tgAloneFinger1N(byte[] fingerTemplData, int fingerSize, Boolean isSound, final VerifyMsg verifyMsg) {
        //3特征模式的话
        if (!TGB2API.this.isLink) return;
        if (!TGB2API.this.isType6) return;
        if (isSound) {
            getAP(mContext).play_inputDownGently();
        } else {
            if (this.soundPlay != null) {
                this.soundPlay.playAudio(Constant.DOWN_GENTLY);
            }
        }
    ...
    private void tgTempl1_1(byte[] templData, final VerifyMsg verifyMsg) {
        if (TGB2API.this.sound) {
            if (TGB2API.this.isLink)
                getAP(mContext).play_inputDownGently();
        } else {
            if (this.soundPlay != null && TGB2API.this.isLink) {
                this.soundPlay.playAudio(Constant.DOWN_GENTLY);
            }
        }
        ...
        if (imgResultCode >= 0) {
            ...
            if (featureResult == 1) {
                ...
                int match1Res = getTGFV().TGFeatureMatchTmpl11(fingerFeatureData, matchData,
                        updateTempl, intByReference);
                if (match1Res == 0) {
                    res = 1;
                    //1:1验证分数
                    int score = intByReference.getValue();
                    if (TGB2API.this.sound && this.isLink) {
    ...
    private void shuntVerifyMethod(@NonNull byte[] fingerFeatureData, @NonNull byte[] fingerTemplData
        ...
        VerifyNBean verifyNBean = tgTempl1_N(fingerFeatureData, matchData, fingerSize);
        ...
        if (res1N == 1) {
            ...
            if (TGB2API.this.sound && TGB2API.this.isLink) {
                getAP(mContext).play_verifySuccess();
            }
            //存在相同的模板
            TGB2API.this.equalData = true;
            callBack.verifyResultCallBack(verifyResult);
        } else if (res1N == 2) {
            //验证失败，分数不及格
            n++;
            ...
            if (!TGB2API.this.equalData && TGB2API.this.sound
                    && TGB2API.this.n == cellDataCount && TGB2API.this.isLink) {
                getAP(mContext).play_verifyFail();
            }
            callBack.verifyResultCallBack(verifyResult);
        } else if (res1N == -1) {
            ...
            if (!TGB2API.this.equalData && TGB2API.this.sound && TGB2API.this.isLink) {
                getAP(mContext).play_verifyFail();
            }
            callBack.verifyResultCallBack(verifyResult);
        }
    ...
    private Msg tgReadDataFromHost() {
        Msg msg = null;
        byte[] data;
        if (!TextUtils.isEmpty(TGB2API.this.savePath)) {
    ...
    private Msg tgSaveFileToHost() {
        Msg msg = null;
        if (TGB2API.this.saveData != null && TGB2API.this.saveData.length > 0
                && !TextUtils.isEmpty(TGB2API.this.savePath)) {
    ...
    private Msg tgCancelRegister() {
        ...
        if (tgCancelRegisterRes == 0) {
            TGB2API.this.isCancelRegister = true;
            msg = new Msg(1, "取消成功");
    ...
    private int checkRepeat(byte[] templData, int templSize) {
        ...
        if (imgResultCode >= 0) {
            ...
            if (featureResult == 1) {
                ...
                VerifyNBean verifyNBean = tgTempl1_N(fingerFeatureData, matchData, templSize);
                int res1N = verifyNBean.getVerifyNResult();
                if (res1N == 1) {
                    //验证成功，模板登记重复
                    res = 7;
                    if (TGB2API.this.isLink && TGB2API.this.sound)
...
//                        }
    ...
    private FingerImgBean tgDevGetFingerImg(int soundType) {
        byte[] imgData;
        if (sendImg) {
            imgData = new byte[TGB2Constant.IMG_SIZE + TGB2Constant.T_SIZE];
            imgData[0] = ((byte) 0xfe);
        } else {
            imgData = new byte[TGB2Constant.IMG_SIZE];
        }
        //抓图
        int DevImageLength = getTG661().TGGetDevImage(imgData, TGB2Constant.GET_IMG_OUT_TIME);
        if (DevImageLength >= 0) {
    ...
    private FingerFeatureBean extractImgFeature(byte[] fingerImgData) {
        byte[] fingerFeature = new byte[TGB2Constant.FEATURE_SIZE];
        int extractFeatureRes = getTGFV().TGImgExtractFeatureRegister(fingerImgData,
                TGB2Constant.IMG_W, TGB2Constant.IMG_H, fingerFeature);
    ...
    public byte[] resolveAllTempl(byte[] fingerTemplData, int fingerSize) {
        ...
        for (int i = 0; i < fingerSize; i++) {
            ...
            byte[] finger = null;
            if (templModelType == TGB2Constant.TEMPL_MODEL_3) {
                finger = new byte[TGB2Constant.PERFECT_FEATURE_3];
            } else if (templModelType == TGB2Constant.TEMPL_MODEL_6) {
                finger = new byte[TGB2Constant.PERFECT_FEATURE_6];
            }
            int fingerLength = 0;
            if (templModelType == TGB2Constant.TEMPL_MODEL_3) {
                fingerLength = TGB2Constant.PERFECT_FEATURE_3;
            } else if (templModelType == TGB2Constant.TEMPL_MODEL_6) {
                fingerLength = TGB2Constant.PERFECT_FEATURE_6;
            }
    ...
    private byte[] jointTempl(byte[] newFeature) {
        if (aimByte == null) {
            if (templModelType == TGB2Constant.TEMPL_MODEL_3) {
                aimByte = new byte[TGB2Constant.FEATURE_SIZE * templModelType];
            } else if (templModelType == TGB2Constant.TEMPL_MODEL_6) {
                aimByte = new byte[TGB2Constant.FEATURE_SIZE * templModelType];
            }
        }
        int length = templIndex * TGB2Constant.FEATURE_SIZE;
    ...
    private RegisterResult registerFinger(int templTypeSize) {
        ...
        for (int i = 0; i < templTypeSize; i++) {
            ...
            if (templIndex == 0 && TGB2API.this.isLink) {
                ...
                if (this.soundPlay != null) {
                    this.soundPlay.playAudio(Constant.DOWN_AGAIN);
                } else {
                    if (TGB2API.this.sound)
            ...
            if (imgResultCode >= 0) {
                ...
                if (result == 1) {
                    ...
                    int size = templModelType == TGB2Constant.TEMPL_MODEL_6 ? 6 : 3;
                    if (templIndex == size) {
                        //特征融合
    ...
    private byte[] resolveTempl(byte[] oldMatchTemplData) {
        ...
        if (oldMatchTemplData != null) {
            ...
            if (templModelType == TGB2Constant.TEMPL_MODEL_3) {
                matchTemplData = new byte[TGB2Constant.WAIT_COMPARE_FEATURE_3];
            } else if (templModelType == TGB2Constant.TEMPL_MODEL_6) {
                matchTemplData = new byte[TGB2Constant.WAIT_COMPARE_FEATURE_6];
            }
    ...
    private byte[] perfectTemplData() {
        ...
        if (templModelType == TGB2Constant.TEMPL_MODEL_3) {
            updateTempl = new byte[TGB2Constant.PERFECT_FEATURE_3];
        } else if (templModelType == TGB2Constant.TEMPL_MODEL_6) {
            updateTempl = new byte[TGB2Constant.PERFECT_FEATURE_6];
        }
        return updateTempl;
    }
    ...
    private byte[] comparableTemplData(int featureSize) {
        byte[] matchData = null;
        if (templModelType == TGB2Constant.TEMPL_MODEL_3) {
            matchData = new byte[TGB2Constant.WAIT_COMPARE_FEATURE_3 * featureSize];
        } else if (templModelType == TGB2Constant.TEMPL_MODEL_6) {
            matchData = new byte[TGB2Constant.WAIT_COMPARE_FEATURE_6 * featureSize];
        }
        return matchData;
    }
     ...
     * -3:设备打开成功，入参错误
     */
    private Msg tgOpenDev() {
        IntByReference mode = new IntByReference();
        int openDevRes = getTG661().TGOpenDev(mode);
        LogUtils.d("打开设备    调用结束    结果：" + openDevRes);
        if (openDevRes >= 0) {
            //设置工作模式
            devOpen = true;
            openDevRes = 1;
            TGB2API.this.isLink = true;
        } else {
            boolean loopOpen = true;
            int n = 0;
            while (loopOpen) {
                try {
                    LogUtils.d("   打开设备  循环调用  ");
                    Thread.sleep(100);
        ...
        if (openDevRes == 1) {
            ...
            myHandler.post(new Runnable() {
                ...
                public void run() {
                    if (devStatusCallBack != null)
                        devStatusCallBack.devStatus(msg);
                }
            });
            ...
            executor.execute(new Runnable() {
                ...
                public void run() {
                    if (inputStream != null) {
                        ...
                        //检测指定的文件夹下是否存在证书
                        ArrayList<String> licenseList = FileUtil.getInitFinerFileList(TGB2Constant.LICENSE_DIR);
                        if (licenseList != null) {
                            ...
                            createDirPath(TGB2Constant.LICENSE_DIR);
                            licenceRes = writeLicense();
                        }
                    }
                    if (licenceRes == 1) {
    ...
    private Msg getFvVersion() {
        byte[] fvVersion = new byte[TGB2Constant.FV_VERSION];
    ...
    private int FV_InitAct() {
        int tgInitFVProcessRes = getTGFV().TGInitFVProcess(TGB2Constant.LICENSE_PATH);
    ...
    private byte[] cellDataSpace(int size) {
        ...
        if (templModelType == TGB2Constant.TEMPL_MODEL_3) {
            cellFingerData = new byte[size * TGB2Constant.PERFECT_FEATURE_3];
        } else if (templModelType == TGB2Constant.TEMPL_MODEL_6) {
            cellFingerData = new byte[size * TGB2Constant.PERFECT_FEATURE_6];
        }
    ...
    private int startPos(int verifyBaseCount, int index) {
        if (templModelType == TGB2Constant.TEMPL_MODEL_3) {
            return verifyBaseCount * TGB2Constant.PERFECT_FEATURE_3 * index;
        } else if (templModelType == TGB2Constant.TEMPL_MODEL_6) {
            return verifyBaseCount * TGB2Constant.PERFECT_FEATURE_6 * index;
        } else {
            return 0;
        }
    }
    ...
    private void createAimDirs() {
        createDirPath(TGB2Constant.LOG_DIR);
        createDirPath(TGB2Constant.LICENSE_DIR);
        createDirPath(TGB2Constant.IMG_PATH);
        createDirPath(TGB2Constant.MONI_EXTER_3_PATH);
        createDirPath(TGB2Constant.MONI_EXTER_6_PATH);
    }
    ...
    private int writeLicenseToFile(InputStream inputStream) {
        ...
        try {
            if (inputStream != null) {
                output = new FileOutputStream(TGB2Constant.LICENSE_PATH);
                // 拷贝到输出流
