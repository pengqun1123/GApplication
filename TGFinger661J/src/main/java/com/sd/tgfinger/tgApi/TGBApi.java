 ...
 * 封装的后比
 * 小特征专用类
 * 使用activity.runOnUiThread()来切换线程
 * <p>
 * 当前的算法版本是V3.0.0.5
 */
public class TGBApi {
    ...
    //获取代理对象
    private static TGBApi tgapi = null;

    public static TGBApi getTGAPI() {
        if (tgapi == null) {
            synchronized (TGBApi.class) {
                if (tgapi == null) {
                    tgapi = new TGBApi();
                }
            }
    ...
    //SDK的当前版本号
    private static final String SDK_VERSION = "1.2.5_190923_Beta";
    ...
    //获取算法的版本号
    public void fvVersion(final Activity activity, final FVVersionCallBack fvVersionCallBack) {
    ...
    //打开设备
    public void openDev(@NonNull final Activity activity, @NonNull Integer workType,
                        @NonNull Integer templModelType, boolean sound,
                        final DevOpenCallBack callBack, DevStatusCallBack devStatusCallBack) {
     ...
     * 关闭指静脉设备
     */
    public void closeDev(@NonNull final Activity activity, final DevCloseCallBack callBack) {
        this.activity = activity;
     ...
     * -3 ：入参错误
     */
    public void setDevWorkModel(@NonNull final Activity activity, @NonNull Integer workType,
                                final DevModelCallBack callBack) {
        this.activity = activity;
        this.workType = workType;
        if (executor != null) {
            executor.execute(new Runnable() {
    ...
    public void setVerifyGetImgListener(VerifyGetFingerImgListener verifyGetImgListener) {
     ...
     * 获取设备的工作模式
     */
    public void getDevWorkModel(@NonNull final Activity activity,
                                final DevWorkModelCallBack callBack) {
        this.activity = activity;
        if (executor != null) {
            executor.execute(new Runnable() {
                @Override
     ...
     * 获取设备的连接状态
     */
    public void getDevStatus(@NonNull final Activity activity, final DevStatusCallBack callBack) {
        this.activity = activity;
        if (executor != null) {
            executor.execute(new Runnable() {
                @Override
     ...
     * 注册
     */
    public void extractFeatureRegister(@NonNull final Activity activity,
                                       @NonNull final byte[] verifyFingerData
            , @NonNull final Integer verifyFingerSize
    ...
    //取消抓图或注册
    public void cancelRegisterGetImg(@NonNull final Activity activity, final CancelImgCallBack callBack) {
        this.activity = activity;
     ...
     * 获取设备的固件号
     */
    public void devFw(@NonNull final Activity activity, final DevFwCallBack callBack) {
        this.activity = activity;
        if (executor != null) {
            executor.execute(new Runnable() {
                @Override
    ...
    public void saveDataToHost(@NonNull final Activity activity,
                               @NonNull byte[] saveData, @NonNull String SavePath
            , final DataSaveCallBack callBack) {
        this.activity = activity;
     ...
     * 从主机设备读取数据
     */
    public void readDataFromHost(@NonNull final Activity activity, @NonNull String sourcePath
     ...
     * 1:N验证
     */
    public void featureCompare1_N(@NonNull final Activity activity,
                                  @NonNull final byte[] verifyData,
                                  final int verifySize, final Verify1_NCallBack callBack) {
        this.activity = activity;
        this.continueVerify = false;
        if (executor != null) {
            executor.execute(new Runnable() {
                @Override
                public void run() {
                    aloneVerifyN(verifyData, verifySize, TGBApi.this.sound, new VerifyMsg() {
                        @Override
     ...
     * templData 模板数据
     */
    public void featureCompare1_1(@NonNull final Activity activity,
                                  @NonNull final byte[] verifyData
            , final Verify1_1CallBack callBack) {
        this.activity = activity;
        if (executor != null) {
            executor.execute(new Runnable() {
                @Override
    ...
    private void aloneVerifyN(@NonNull byte[] fingerTemplData, int fingerSize, Boolean isSound, VerifyMsg verifyMsg) {
        if (!TGBApi.this.continueVerify && fingerSize > 0) {
    ...
    private void tgAloneFinger1N(byte[] fingerTemplData, int fingerSize, Boolean isSound, final VerifyMsg verifyMsg) {
        //3特征模式的话
        if (!TGBApi.this.isLink) return;
        if (!TGBApi.this.isType6) return;
        if (isSound && TGBApi.this.isLink) {
     ...
     *                  -10:登记失败
     */
    private void tgTempl1_1(byte[] templData/*, boolean sound*/, final VerifyMsg verifyMsg) {
        if (TGBApi.this.sound && TGBApi.this.isLink)
            getAP(mContext).play_inputDownGently();
    ...
    private void shuntVerifyMethod(@NonNull byte[] fingerFeatureData, @NonNull byte[] fingerTemplData
        ...
        if (res1N == 1) {
            ...
            if (!TGBApi.this.equalData && TGBApi.this.sound
                    && TGBApi.this.n == cellDataCount && TGBApi.this.isLink) {
                getAP(mContext).play_verifyFail();
            }
            callBack.verifyResultCallBack(verifyResult);
        } else if (res1N == -1) {
            ...
            if (!TGBApi.this.equalData && TGBApi.this.sound && TGBApi.this.isLink) {
                getAP(mContext).play_verifyFail();
            }
            callBack.verifyResultCallBack(verifyResult);
        }
    ...
    private Msg tgReadDataFromHost() {
        Msg msg = null;
        byte[] data;
        if (!TextUtils.isEmpty(TGBApi.this.savePath)) {
    ...
    private Msg tgSaveFileToHost() {
        Msg msg = null;
        if (TGBApi.this.saveData != null && TGBApi.this.saveData.length > 0
                && !TextUtils.isEmpty(TGBApi.this.savePath)) {
    ...
    private Msg tgCancelRegister() {
        ...
        if (tgCancelRegisterRes == 0) {
            TGBApi.this.isCancelRegister = true;
            msg = new Msg(1, "取消成功");
        ...
        if (imgResultCode >= 0) {
            ...
            if (featureResult == 1) {
                ...
                if (res1N == 1) {
                    //验证成功，模板登记重复
                    res = 7;
                    if (TGBApi.this.isLink && TGBApi.this.sound)
...
//                        }
    ...
    private RegisterResult registerFinger(int templTypeSize) {
        ...
        for (int i = 0; i < templTypeSize; i++) {
            ...
            if (templIndex == 0 && TGBApi.this.isLink && TGBApi.this.sound) {
                getAP(mContext).play_inputDownGently();
            } else if (templIndex > 0 && this.isLink && TGBApi.this.sound) {
