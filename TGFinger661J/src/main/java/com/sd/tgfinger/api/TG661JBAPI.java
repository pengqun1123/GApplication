...
//    public static final int WAIT_COMPARE_FEATURE_6 = 6432;//3特征
    ...
    public static final int WAIT_COMPARE_FEATURE_3 = 3248;//3特征
    public static final int WAIT_COMPARE_FEATURE_6 = 6464;//6特征
...
//        InitLicense();
    ...
    //所有的已存在的模板数据
    private byte[] allTemplData = null;
    private int countSize;

    //外部调用，更新缓存中的模板数据
    public void updateAllTemplData(byte[] newCountTemplData, int countSize, int templSources) {
...
//        this.userFingerName = userName;
    ...
    private void sendEmptyDataMsg(Handler handler) {
...
//            if (TextUtils.isEmpty(templNameID))
        ...
        //注册前核对当前指静脉是否已经注册
        if (allFingerMatchDatas != null && allFingerMatchDatas.length > 0 && !isCheck && !hasTemplName) {
    ...
    private void readFingerAndExchanged(File[] files, int startRead, int readLength
        ...
        if (files.length > 0) {
            ...
            for (int i = 0; i < readLength; i++) {
                ...
                System.arraycopy(bytes, 0, lengthByte,
                        compareTemplLength, bytes.length);
            }
...
//                                                }
                    ...
                    templCount = countSize;
                    allFingerMatchDatas = allTemplData;
                }
                return null;
            }
        });
                            ...
                            public void run() {
                                ...
                                if (tgFeatureMatchTmpl1NRes == 0) {
                                    ...
                                    if (templSources == DIR_TEMPL_SOURCES) {
                                        ...
                                        bundle.putInt(INDEX, templIndex - 1);
                                    }
    ...
    public void getAllFingerData2() {
        ...
        if (allTemplByteList != null && allTemplByteList.size() > 0) {
            ...
            for (int i = 0; i < templCount; i++) {
                ...
                final byte[] allFingerMatchDatas1 = jointTempls(templData, i);
                if (i == templCount - 1) {
                    mActivity.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
...
//                        if (allFingerMatchDatas[0] == 0 && allFingerMatchDatas[1] == 0) {
//                            isCheck = true;
//                            hasTempl = false;
//                            int firstRegister = getTGFV().TGImgExtractFeatureRegister(
//                                    match1_NImgData, IMG_W, IMG_H, match1_NFeature);
...
//        String command = "irtest -f -S -d /dev/hidraw0";
//        String command = "chmod 777 /dev/hidraw1";
        try {
            Process process = Runtime.getRuntime().exec(new String[]{"su", "-c", command});
            int i = process.waitFor();
