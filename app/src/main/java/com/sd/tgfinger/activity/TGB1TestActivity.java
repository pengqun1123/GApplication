 ...
 * 大特征示例demo
 */
public class TGB1TestActivity extends AppCompatActivity implements DevOpenCallBack, DevStatusCallBack,
        View.OnClickListener, SoundPlay, FingerVerifyResultListener {
...
//            LogUtils.d("打开设备：调用  11");
            boolean sound = true;
            tgapi.openDev(TGB2Constant.WORK_BEHIND, TGB2Constant.TEMPL_MODEL_6,
                    sound, this, this);
            if (sound) {
...
//        }
    ...
    private void readData() {
        alertDialog1 = AlertDialogUtil.Instance().showWaitDialog(TGB1TestActivity.this, "数据准备中...");
        if (templType == TGB2Constant.TEMPL_MODEL_6) {
            tgapi.readDataFromHost(TGB2Constant.MONI_EXTER_6_PATH, new ReadDataCallBack() {
                @Override
    ...
    private void initView() {
        ...
        getCurrentVoice();
        //默认特征模式为6模板
        templType = TGB2Constant.TEMPL_MODEL_6;
    ...
    private void statusSetting() {
        ...
        templSumModel.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(RadioGroup radioGroup, int i) {
                if (templ3Rb.getId() == i) {
                    templType = TGB2Constant.TEMPL_MODEL_3;
                    tgapi.setTemplModelType(TGB2Constant.TEMPL_MODEL_3);
                    //初始化数据，开启连续验证
//                    readFingerData(3);
                    readData();
                    ...
                    templType = TGB2Constant.TEMPL_MODEL_6;
                    tgapi.setTemplModelType(TGB2Constant.TEMPL_MODEL_6);
                    //初始化数据，开启连续验证
                    //readFingerData(3);
                    readData();
                }
    ...
    private void verify() {
        ...
        String path = TGB2Constant.MONI_EXTER_6_PATH + File.separator + fingerName;
        List<String> allFileName = FileUtil.getAllFileName(TGB2Constant.MONI_EXTER_6_PATH);
        boolean no = false;
    ...
    private void updateFingerData(Integer index, @NonNull byte[] updateFingerData) {
        //根据index更新模板库的模板
        if (templType == TGB2Constant.TEMPL_MODEL_3) {

        } else if (templType == TGB2Constant.TEMPL_MODEL_6) {
...
//                    FingerDataUtil.getInstance().insertOrReplaceFinger(fingerData);
    ...
    @Override
    public void playAudio(Integer type) {
        LogUtils.d("  语音回调  ");
        if (type.equals(Constant.DOWN_AGAIN)) {
            LogUtils.d("播放语音  11");
            tgapi.getAP(this).play_input_again();
        } else if (type.equals(Constant.DOWN_GENTLY)) {
            LogUtils.d("播放语音  22");
            tgapi.getAP(this).play_inputDownGently();
        }
    }
            ...
            if (!isCancelVerify) {
                ...
                tgapi.featureCompare1_N(finger6Data, finger6Size, new Verify1_NCallBack() {
                    ...
                    public void verify1_NCallBack(Msg msg) {
                        ...
                        LogUtils.d("验证的结果:" + result);
                        if (result == 1) {
                            tgapi.getAP(TGB1TestActivity.this).play_verifySuccess();
                            //比对的分数
                            Integer score = msg.getScore();
                            //比对模板对应的位置
                            Integer index = msg.getIndex();
                            ...
                            tgapi.getAP(TGB1TestActivity.this).play_time_out();
                            showTip(msg.getTip());
                            tgapi.cancelRegisterGetImg(new CancelImgCallBack() {
                                @Override
                                public void cancelImgCallBack(Msg msg) {
                            ...
                            tgapi.getAP(TGB1TestActivity.this).play_verifyFail();
                            showTip(msg.getTip());
                        }
                    }
                });
